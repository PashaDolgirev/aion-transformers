# right after multi-head self-attention, we add a feed-forward NN that acts on each token separately and identically
...
class FullModel(nn.Module):
    # progressively build the full language model --> here, we add feed-forward NN after multi-head self-attention

    def __init__(self, num_heads=4, n_hidden=4*n_embd):
        super().__init__()
        ...
        self.sa_heads = MultiHead(num_heads, head_size=n_embd // num_heads) # multi-head self-attention
        self.ffwd = FeedForward(n_embd, n_hidden=n_hidden) # feed-forward layer, acts on each token separately and identically
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.sa_heads(x) # self-attention layer 
        x = self.ffwd(x) # feed-forward layer, acts on each token separately and identically
