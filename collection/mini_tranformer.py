...
max_iters = 5000
eval_interval = 500 # used for estimating loss
learning_rate = 3e-4
...
class FeedForward(nn.Module):
    #a sinle linear layer followed by ReLu 
...
class SimpleOneHeadAttentionModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.sa_head = Head(n_embd) # one head of self-attention
...
class Block(nn.Module):
    """attention block followed by a nn layer"""
...
class MultiHeadAttentionModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.token_embedding_table = nn.Embedding(vocab_size, n_embd)
        self.position_embedding_table = nn.Embedding(block_size, n_embd)
        self.sa_heads = MultiHead(num_heads=4, head_size=n_embd // 4) # multi-head self-attention
        self.lm_head = nn.Linear(n_embd, vocab_size)
...
class MultiHeadAttentionModelWFF(nn.Module):
    """Multi head attention model with feed forward layer"""
...
class MultiHeadAttnModelBlockWFF(nn.Module):
    """Blocks of attention followed by feed forward layer"""
...
model = MultiHeadAttnModelBlockWFF()
